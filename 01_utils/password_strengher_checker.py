...
def check_password_strengh(password):
    ...
    if not any(c.islower() for c in password):
        issues.append("Password should contain at least one lower case letter")
    if not any(c.isupper() for c in password):
        issues.append("Password should contain at least one upper case letter")
    if not any(c.isdigit() for c in password):
        issues.append("Password should contain at least one number")
    if not any(c in string.punctuation for c in password):
        issues.append("Password should contain at least one spcial character")
    return issues
...
issues = check_password_strengh(password)
