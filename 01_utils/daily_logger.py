...
import datetime
entry = input("What did you learn today? ").strip().capitalize()
rating = input("✨ Rate your productivity today from 1 to 5 (opitional): ").strip()

now = datetime.datetime.now()
date_str = now.strftime("%d/%m/%Y - %H:%M")
journal_entry = f"\n 📅 {date_str}\n{entry}"
if rating:
    journal_entry += f"\n Produtictivity Rating: {rating}\n"
journal_entry += "\n" + "-" * 50
